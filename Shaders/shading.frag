...
uniform vec4 scene_light = vec4(1.0,1.0,1.0,1.0);
...
void main() 
{
	...
	float attenuation = diffuseReflectance * visibility;
	...
	//vec3 reflectionVector = (inverseViewNormalMatrix * vec4(directionFromEye -2 * dot(directionFromEye, normal) * normal , 0.0)).xyz;
	vec3 envMapSample = texture(cubeMap, reflectionVector).rgb;

	vec3 shading = calculateAmbient(scene_ambient_light, ambient)
		+ calculateDiffuse(vec3(scene_light.xyz), diffuse, normal, directionToLight) * visibility;
		+ calculateSpecular(vec3(scene_light.xyz), diffuse, material_shininess, normal, directionToLight, directionFromEye) * visibility
		+ emissive
