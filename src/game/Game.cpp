...
//Make a room and add it to the scene
...
//create a room object from a pixel of the ppm according to its neighbors
...
//Load the maze from the ppm file
...
//Check collision between actors of the scene and the player
...
//Load the entire world from the .txt file
...
//Check if the player enters one of the monster aggro radius
...
int Game::mainLoop()
{
    ...
    endAABB.max.y = 1;
    //Load shaders
    Shader* lightingShader = new Shader("../Shaders/simpleLight" );
    ...
    Text* instructions = new Text ( "Find the exit key among the chests without dying", glm::vec2(20.0, 200.0), 30, false);
    ...
    /*
      The main loop, this is where the magic is happening each frame :
    ...
    */
    while(!endgame)
    {
        if ( !gameOver && !winGame )
        {
            ...
            framerate_t->setText(to_string(framerate));
...
//SET LIGHTING SHADER UNIFORMS
            lightingShader->use();
// Transformation matrices                
            glUniformMatrix4fv(glGetUniformLocation(lightingShader->getProgram(), "projection"),
                               1, GL_FALSE, glm::value_ptr(projection));
            glUniformMatrix4fv(glGetUniformLocation(lightingShader->getProgram(), "view"),
                               1, GL_FALSE, glm::value_ptr(view));
// Set the lighting uniforms
            glUniform3f(glGetUniformLocation(lightingShader->getProgram(), "viewPos"),
                        p->getCamera()->getPosition().x,p->getCamera()->getPosition().y, p->getCamera()->getPosition().z);
            glUniform3f(glGetUniformLocation(lightingShader->getProgram(), "lightPos"),
                        p->getCamera()->getPosition().x,p->getCamera()->getPosition().y, p->getCamera()->getPosition().z);
// Point light 1
            glUniform3f(glGetUniformLocation(lightingShader->getProgram(), "pointLights[0].position"),
                        ...
                        p->getCamera()->getPosition().y,
                        p->getCamera()->getPosition().z);     
            glUniform3f(glGetUniformLocation(lightingShader->getProgram(), "pointLights[0].ambient"),
                        0.05f, 0.05f, 0.05f);       
            glUniform3f(glGetUniformLocation(lightingShader->getProgram(), "pointLights[0].diffuse"),
                        1.0f, 1.0f, 1.0f); 
            glUniform3f(glGetUniformLocation(lightingShader->getProgram(), "pointLights[0].specular"),
                        1.0f, 1.0f, 1.0f);
            glUniform1f(glGetUniformLocation(lightingShader->getProgram(), "pointLights[0].constant"),
                        1.0f);
            glUniform1f(glGetUniformLocation(lightingShader->getProgram(), "pointLights[0].linear"),
                        0.005);
            glUniform1f(glGetUniformLocation(lightingShader->getProgram(), "pointLights[0].quadratic"),
                        1.0);  

            GLint maxTex;
            glGetIntegerv(GL_MAX_COMBINED_TEXTURE_IMAGE_UNITS, &maxTex);
            glActiveTexture(GL_TEXTURE0 + --maxTex);       
            glUniform1i(glGetUniformLocation(lightingShader->getProgram(), "shadowMap"), maxTex);
            glBindTexture(GL_TEXTURE_2D, depthMapTex);          
...
//RENDER WATER REFLECTION
            ...
            glUniform4f(glGetUniformLocation(lightingShader->getProgram(), "clipPlane"),
            ...
            glUniformMatrix4fv(glGetUniformLocation(lightingShader->getProgram(), "invert"),
            ...
            drawScene(scene, lightingShader, 0);
            glCullFace(GL_BACK);
            invert = glm::mat4(1.0);
            glUniformMatrix4fv(glGetUniformLocation(lightingShader->getProgram(), "invert"),
                               1, GL_FALSE, glm::value_ptr(invert));
...
//RENDER SCENE
            ...
            drawScene(scene, lightingShader, waterShader);
